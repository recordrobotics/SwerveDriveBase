...
public class PushAmp extends SequentialCommandGroup {
  ...
  /**
   * Command that assumes the flywheel and crashbar are already in position and moves the channel to
   * shoot the note into the amp. Then, turns everything off.
   *
   * @param channel
   * @param shooter
   * @param crashbar
  ...
  public PushAmp(Channel channel, Shooter shooter, Crashbar crashbar, Double shootTime) {
    ...
    addCommands(
        ...
        new InstantCommand(() -> _channel.toggle(ChannelStates.OFF), _channel)
            .handleInterrupt(killSpecified),
        new InstantCommand(() -> _crashbar.toggle(CrashbarStates.RETRACTED), _crashbar)
            .handleInterrupt(killSpecified));
  }
