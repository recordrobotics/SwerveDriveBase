 ...
 *
 * <p>This class simulates the two <strong>REEF</strong>s on the field where <strong>CORAL</strong>s
 * can be scored. It includes all 12 {@link ReefscapeReefBranchesTower} instances on the field (both
 * blue and red).
 */
public class ImprovedReefscapeReefSimulation implements SimulatedArena.Simulatable {
  private final ImprovedArena2025Reefscape arena;
  private final List<ReefscapeReefBranchesTower> branchTowers;
  private final List<CoralHolder> coralHolders;

  public ImprovedReefscapeReefSimulation(ImprovedArena2025Reefscape arena) {
    this.arena = arena;
    ...
    for (int i = 0; i < 12; i++) {
      // blue
      branch =
          new ReefscapeReefBranchesTower(
              branchesCenterPositionBlue[i], branchesFacingOutwardsBlue[i]);
      ...
      branch =
          new ReefscapeReefBranchesTower(
              branchesCenterPositionRed[i], branchesFacingOutwardsRed[i]);
      branchTowers.add(branch);
      coralHolders.addAll(branch.coralHolders());
    }
  }
   ...
   *
   * @param coralPosesToDisplay a list of {@link Pose3d} objects used to visualize the positions of
   *     the CORALs on AdvantageScope
   */
   ...
   *
   * <p>The <strong>BRANCHES</strong> are tracked in FMS as A, B, C, D, E, F, G, H, I, J, K, L (as
   * per the game manual), and are mapped to indices 0, 1, 2, ... in the array.
   *
   * <p>The [i][j] entry in the array represents the number of <strong>CORAL</strong>(s) held on the
   * L<code>j-1</code> branch in the <code>i</code>th section.
   *
   * <p>For example, <code>getBranches()[2][3]</code> returns the number of CORALs held on L4 of
   * Branch C.
   *
   * <p>Note that L2, L3, and L4 can only hold one <strong>CORAL</strong>, while L1 can hold up to
   * two <strong>CORAL</strong>s.
   *
   * @param side the alliance side (Red or Blue) to check for CORAL counts
   * @return a 2D array where each entry represents the number of <strong>CORAL</strong> held on
   *     each branch
   */
  public int[][] getBranches(DriverStation.Alliance side) {
    int[][] coralsCountOnBranches = new int[12][4];
    for (int i = 0; i < 12; i++) {
      ReefscapeReefBranchesTower tower =
          branchTowers.get(side == DriverStation.Alliance.Red ? i * 2 + 1 : i * 2);
   ...
   */
  public static Optional<ImprovedReefscapeReefSimulation> getInstance() {
    if (SimulatedArena.getInstance() instanceof ImprovedArena2025Reefscape arena2025Reefscape)
...
class ReefscapeReefBranchesTower {
  ...
  public ReefscapeReefBranchesTower(
      Translation2d stickCenterPositionOnField, Rotation2d facingOutwards) {
    // L1 trough, 15cm away from center
    this.L1 =
        new ReefscapeReefTrough(
 ...
 *
 * <p>If a <strong>CORAL</strong> ejected into the air is in contact with this
 * <strong>BRANCH</strong> at the correct angle, as detected by {@link
 * #checkCoralPlacement(ReefscapeCoralOnFly)}, it will get attached.
 */
final class ReefscapeReefBranch implements CoralHolder {
  ...
  private static final double TRANSLATIONAL_TOLERANCE_METERS = 0.13;
  private static final double ROTATIONAL_TOLERANCE_RADIANS = Math.toRadians(60);
   ...
   *   <li>The <strong>CORAL</strong> is detected to be in physical contact with the BRANCH in
   *       space, within a specified tolerance.
   *   <li>The <strong>CORAL</strong>'s opening is aligned with the direction of the BRANCH's tip,
   *       ensuring proper placement.
   *   <li>The <strong>CORAL</strong> is moving in a direction that is towards the inside of the
   *       BRANCH, not away from it.
   * </ul>
   *
   * <p>In more technical terms, the CORAL's placement is validated by checking if the distance
   * between the CORAL's current position and the ideal placement pose is within a predefined
   * tolerance. Additionally, the CORAL's velocity and direction of movement are checked to ensure
   * it is moving towards the correct placement orientation.
   *
   ...
   * @return <code>true</code> if the CORAL is successfully placed onto this BRANCH, <code>false
   *     </code> otherwise
   */
  @Override
  public boolean checkCoralPlacement(ReefscapeCoralOnFly coralOnFly) {
    ...
    Translation3d idealCoralFacingVector =
        new Translation3d(1, idealCoralPlacementPose.getRotation());
    Translation3d actualCoralFacingVector =
        new Translation3d(1, coralOnFly.getPose3d().getRotation());
    double rotationToleranceVectorNorm =
        new Translation2d(1, Rotation2d.fromRadians(ROTATIONAL_TOLERANCE_RADIANS))
            .minus(new Translation2d(1, 0))
            .getNorm();
    boolean directionCorrect =
        idealCoralFacingVector.minus(actualCoralFacingVector).getNorm()
                < rotationToleranceVectorNorm
            || idealCoralFacingVector.times(-1).minus(actualCoralFacingVector).getNorm()
                < rotationToleranceVectorNorm;
 ...
 *
 * <p>If a <strong>CORAL</strong> ejected into the air is in contact with this
 * <strong>TROUGH</strong> at the correct angle, as detected by {@link
 * #checkCoralPlacement(ReefscapeCoralOnFly)}, it will get attached.
 */
final class ReefscapeReefTrough implements CoralHolder {
  ...
  protected ReefscapeReefTrough(Translation2d centerPosition, Rotation2d outwardsFacing) {
    ...
    this.idealPlacementPosition =
        new Translation3d(centerPosition.getX(), centerPosition.getY(), 0.47);
    this.coralCount = 0;
  }
   ...
   *
   * <p>A CORAL is successfully placed if it is within a specified distance from the ideal placement
   * position and within a reasonable height range.
   *
   * <p>The placement is considered successful if the following conditions are met:
   *
   * <ul>
   *   <li>The CORAL is close enough to the ideal placement position (within 0.25 units horizontally
   *       and 0.1 units vertically).
  ...
  public boolean checkCoralPlacement(ReefscapeCoralOnFly coralOnFly) {
    ...
    Translation3d difference =
        coralOnFly.getPose3d().getTranslation().minus(idealPlacementPosition);
    boolean closeEnough =
        difference.toTranslation2d().getNorm() < 0.25 && Math.abs(difference.getZ()) < 0.1;
